  ...
  return {
    ...
    keywords: {
      keyword:
        ...
        'CONSTANTS CONTINUE CORRESPONDING CREATE CROSS DATA DEFAULT DEFINE DELETE DESCENDING DISTINCT DO ELSE ELSEIF ENDCASE ENDCLASS ENDDO ENDFORM ' +
        'ENDIF ENDLOOP ENDMETHOD ENDSELECT ENDTRY ENDWHILE EXIT EXPORTING FIELD-SYMBOLS FOR FORM FROM FUNCTION IF IMPORTING ' +
        'IN INCLUDE INNER INSERT INTO JOIN LEFT LOOP MESSAGE METHOD METHODS MODIFY MOVE NEW OBJECT OPTIONAL ORDER OUTER PARAMETERS ' +
        'PERFORM PRIVATE PROTECTED PUBLIC RAISE READ REDUCE REFRESH REPORT RETURN RIGHT ROLLBACK SELECT SELECT-OPTIONS SET ' +
        'SORT SPLIT START-OF-SELECTION SWITCH TABLE TABLES THEN TIMES TRY TYPE TYPES UP USING VALUE WHEN WHERE WHILE WITH WRITE',
